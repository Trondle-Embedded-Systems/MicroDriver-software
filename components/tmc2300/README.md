...
Append the commit hash to `source` if you wish to continue with the previous version. The rest can remain as is.
```yaml
external_components:
  - source: github://Trondle-Embedded-Systems/MicroDriver-software@0035ce5
    components: ...
```
...
For ESP32-S3, use the `esp-idf` framework for full dual-core and IRAM support:
```yaml
external_components:
  - source: github://Trondle-Embedded-Systems/MicroDriver-software
    components: [ tmc2300, stepper ]
